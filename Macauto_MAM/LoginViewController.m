...
- (void)viewDidLoad {
    ...
    [_labelTitle setText:NSLocalizedString(@"MACAUTO_MAM", nil)];
...
- (NSMutableArray *) sendHttpPost {
    _loginlSMAM = [[NSMutableArray alloc] init];
    ...
    _soapMessage = [NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"utf-8\"?>"
                    ...
                    "<soap:Body>"
                    "<login_EDI xmlns=\"http://tempuri.org/\">"
                    "<user_no>%@</user_no>"
                    "<password>%@</password>"
                    "<ime_code>%@</ime_code>"
                    "</login_EDI>"
    ...
    [theRequest addValue: @"http://tempuri.org/login_EDI" forHTTPHeaderField:@"SOAPAction"];
  ...
  namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"loginResponse"]) {
        _doc = false;
    } else if ([elementName isEqualToString:@"login_EDIResult"]) {
        //_MeetingList = false;
        //_checkno = false;
