...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (int i=0; i<_notifyList.count; i++) {
        ...
        if ([item.customer isEqualToString: temp.customer] &&
            [item.edi_type isEqualToString: temp.edi_type] &&
            [item.interchange_ctrl_id isEqualToString: temp.interchange_ctrl_id] &&
            [temp.sp isEqualToString:@"N"]) {
            [temp setReadSp:@"Y"];
            [self sendHttpPost2:temp.customer addEdi_type:temp.edi_type addInterchange_ctrl_id:temp.interchange_ctrl_id];
        }
    }
    ...
    [UIView animateWithDuration:0.7 animations:^{
        ...
        [lbl_title_show setText: [NSString stringWithFormat:@"%@-%@-%@", item.customer, item.edi_type, item.interchange_ctrl_id]];
        
        [lbl_time_show setText: item.date];
...
- (void) searchTableList {
    ...
    for (NotifyItem *tempItem in _notifyList) {
        ...
        if ([tempItem.customer containsString:searchString ] ||
            [tempItem.edi_type containsString:searchString] ||
            [tempItem.interchange_ctrl_id containsString:searchString] ||
            [tempItem.date containsString:searchString]) {
            //if (result == NSOrderedSame) {
            
            NotifyItem *item = [[NotifyItem alloc] init];
            [item setCustomer:tempItem.customer];
            [item setEdi_type:tempItem.edi_type];
            [item setInterchange_ctrl_id:tempItem.interchange_ctrl_id];
            [item setDate:tempItem.date];
...
- (NSMutableArray *) sendHttpPost {
    ...
    soapMessage = [NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"utf-8\"?>"
                   ...
                   "<soap:Body>"
                   "<Get_EDI_list xmlns=\"http://tempuri.org/\">"
                   "<ime_code>%@</ime_code>"
                   "</Get_EDI_list>"
                   "</soap:Body>"
                   "</soap:Envelope>", uuid];
    ...
    [theRequest addValue: @"http://tempuri.org/Get_EDI_list" forHTTPHeaderField:@"SOAPAction"];
...
- (NSMutableArray *) sendHttpPost2:(NSString *)customer addEdi_type:(NSString *)edi_type addInterchange_ctrl_id:(NSString *) interchange_ctrl_id {
    _updateList = [[NSMutableArray alloc] init];
    
    NSLog(@"customer = %@", customer);
    NSLog(@"edi_type = %@", edi_type);
    ...
    soapMessage = [NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"utf-8\"?>"
                   "<soap:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">"
                   "<soap:Body>"
                   "<Update_EDI_Read_Status xmlns=\"http://tempuri.org/\">"
                   "<customer>%@</customer>"
                   "<edi_type>%@</edi_type>"
                   "<interchange_control_id>%@</interchange_control_id>"
                   "<ime_code>%@</ime_code>"
                   "</Update_EDI_Read_Status>"
                   "</soap:Body>"
                   "</soap:Envelope>", customer, edi_type, interchange_ctrl_id,uuid];
  ...
  namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"DocumentElement"]) {
        ...
        if (_doc && _isNotifyList) {
            NSLog(@"<%@>%@</%@>", _elementStart, _elementValue, _elementEnd);
            [_item setCustomer:_elementValue];
        }
    } else if ([elementName isEqualToString:@"edi_type"]) {
        if (_doc && _isNotifyList) {
            NSLog(@"<%@>%@</%@>", _elementStart, _elementValue, _elementEnd);
            [_item setEdi_type:_elementValue];
        }
    } else if ([elementName isEqualToString:@"interchange_control_id"]) {
        if (_doc && _isNotifyList) {
            NSLog(@"<%@>%@</%@>", _elementStart, _elementValue, _elementEnd);
            [_item setInterchange_ctrl_id:_elementValue];
        }
    } else if ([elementName isEqualToString:@"send_datetime"]) {
        if (_doc && _isNotifyList) {
            NSLog(@"<%@>%@</%@>", _elementStart, _elementValue, _elementEnd);
            
            [_item setDate:_elementValue];
        }
